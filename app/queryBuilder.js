 ...
 */
module.exports.fulltextPageQuery = function({uuid, pageTextArray, trimmedArray, wordCountArray, correctSpellingRatioArray}) {
  // Because cypher sucks sometimes.
  var pageTextArrayWithIndices = pageTextArray.map((page, idx) => ({
    text: page, 
    pageno: idx+1,
    trimmed: trimmedArray[idx],
    wordCount: wordCountArray[idx],
    correctSpellingRatio: correctSpellingRatioArray[idx]
  }))

  var query = new Query();
  query.match("(f:Card {Uuid: $uuid})", {uuid})
  query.foreach("page IN $pages", "MERGE (f)-[:HAS_PAGE]->(p:Page:Card {PageNumber: page.pageno}) ON CREATE SET p.Uuid = apoc.create.uuid() SET p.FullText = page.text, p.Name = f.Name + ' - Page '+page.pageno, f.OcrMethod = $ocrMethod, p.WordCount = page.wordCount, p.CorrectSpellingRatio = page.correctSpellingRatio, p.FullTextKeywords = page.trimmed", {
    pages: pageTextArrayWithIndices, ocrMethod: "tesseract"
  })
  return query;
}