...
var corpus;
try {
  corpus = fs.readFileSync('./config/wordlist.txt', 'utf8').split("\n");
}
catch (err) {
  console.log("Could not find dictionary for spell check. Defaulting.")
  corpus = []
}
 ...
 */
module.exports.removeStopWords = function (string) {
  var x;
  var y;
  var word;
  var stop_word;
...
module.exports.removeStopWordsFromArray = function(strArray) {
  let newArr = [];
  strArray.forEach((str) => {
    if(stop_words.indexOf(str.toLowerCase()) == -1) newArr.push(str)
  })
  return newArr;
}

// Returns the number of words in the wordlist that are spelled correctly.
module.exports.spellCheckList = function(wordList) {
  let numCorrect = 0

  wordList.forEach((word) => {
    if(spellcheck.isCorrect(word)) numCorrect++;
  })
